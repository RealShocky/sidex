...
export const manageExtensionIcon = registerIcon(
	...
	Codicon.gear,
	localize('manageExtensionIcon', "Icon for the 'Manage' action in the theme selection quick pick.")
);
...
class MarketplaceThemesPicker implements IDisposable {
	...
	public openQuickPick(
		...
		selectTheme: (theme: IWorkbenchTheme | undefined, applyTheme: boolean) => void
...
class InstalledThemesPicker {
	...
	public async openQuickPick(picks: QuickPickInput<ThemeItem>[], currentTheme: IWorkbenchTheme) {
		...
		if (this.extensionGalleryService.isEnabled()) {
			if (
				(await this.extensionResourceLoaderService.supportsExtensionGalleryResources()) &&
...
registerAction2(
	class extends Action2 {
		...
		override async run(accessor: ServicesAccessor) {
			...
			const setTheme = (theme: IWorkbenchTheme | undefined, settingsTarget: ThemeSettingTarget) =>
				themeService.setColorTheme(theme as IWorkbenchColorTheme, settingsTarget);
			const getMarketplaceColorThemes = (publisher: string, name: string, version: string) =>
				themeService.getMarketplaceColorThemes(publisher, name, version);
...
registerAction2(
	class extends Action2 {
		...
		override async run(accessor: ServicesAccessor) {
			...
			const setTheme = (theme: IWorkbenchTheme | undefined, settingsTarget: ThemeSettingTarget) =>
				themeService.setFileIconTheme(theme as IWorkbenchFileIconTheme, settingsTarget);
			const getMarketplaceColorThemes = (publisher: string, name: string, version: string) =>
				themeService.getMarketplaceFileIconThemes(publisher, name, version);
...
registerAction2(
	class extends Action2 {
		...
		override async run(accessor: ServicesAccessor) {
			...
			const setTheme = (theme: IWorkbenchTheme | undefined, settingsTarget: ThemeSettingTarget) =>
				themeService.setProductIconTheme(theme as IWorkbenchProductIconTheme, settingsTarget);
			const getMarketplaceColorThemes = (publisher: string, name: string, version: string) =>
				themeService.getMarketplaceProductIconThemes(publisher, name, version);
...
registerAction2(
	class extends Action2 {
		...
		override async run(accessor: ServicesAccessor) {
			...
			const preferredId =
				previousTheme.type === ColorScheme.LIGHT || previousTheme.type === ColorScheme.HIGH_CONTRAST_LIGHT
					? ThemeSettingDefaults.COLOR_THEME_LIGHT
					: ThemeSettingDefaults.COLOR_THEME_DARK;
			...
			disposables.add(
				picker.onDidChangeActive(selected => {
					if (selected[0]) {
