	...
	implements IExtHostTerminalService, ExtHostTerminalServiceShape
{
	...
	private readonly _completionProviders: Map<string, vscode.TerminalCompletionProvider<vscode.TerminalCompletionItem>> =
		new Map();
	private readonly _profileProviders: Map<
		string,
		{ provider: vscode.TerminalProfileProvider; extension: IExtensionDescription }
	> = new Map();
	...
	protected _serializeParentTerminal(
		...
		if (
			...
			hasKey(options.location, { parentTerminal: true })
		) {
			...
			hasKey(internalOptions.location, { splitActiveTerminal: true })
		) {
	...
	public $acceptTerminalOpened(
		...
		const creationOptions: vscode.TerminalOptions = {
			...
			cwd:
				typeof shellLaunchConfigDto.cwd === 'string' ? shellLaunchConfigDto.cwd : URI.revive(shellLaunchConfigDto.cwd),
	...
	public $acceptProcessRequestInitialCwd(id: number): void {
		this._terminalProcesses
			...
			?.getInitialCwd()
			.then(initialCwd =>
				this._proxy.$sendProcessProperty(id, { type: ProcessPropertyType.InitialCwd, value: initialCwd })
			);
	}
	...
	public $acceptProcessRequestCwd(id: number): void {
		this._terminalProcesses
			.get(id)
			?.getCwd()
			.then(cwd => this._proxy.$sendProcessProperty(id, { type: ProcessPropertyType.Cwd, value: cwd }));
	}
	...
	public async $createContributedProfileTerminal(
		...
		const profileOptions =
			hasTerminalTitleProposal && options.titleTemplate && !profile.options.titleTemplate
