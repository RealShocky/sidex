...
import {
	...
	IIndentationRuleDto,
	IInlayHintDto,
	IInlineCompletionChangeHintDto,
	IInlineCompletionModelInfoDto,
	IInlineCompletionProviderOptionDto,
...
export class MainThreadLanguageFeatures extends Disposable implements MainThreadLanguageFeaturesShape {
		...
		this._registrations.set(
			...
			this._languageFeaturesService.multiDocumentHighlightProvider.register(selector, {
				...
				provideMultiDocumentHighlights: (
					...
					token: CancellationToken
				): Promise<Map<URI, languages.DocumentHighlight[]> | undefined> => {
					return this._proxy
						.$provideMultiDocumentHighlights(
							handle,
							model.uri,
			...
			this._languageFeaturesService.renameProvider.register(selector, {
				...
				resolveRenameLocation: supportResolveLocation
					? (
							...
							this._proxy.$resolveRenameLocation(handle, model.uri, position, token)
					: undefined
			})
		);
	}
	...
	$registerDocumentSemanticTokensProvider(
		...
		this._registrations.set(
			...
			this._languageFeaturesService.documentSemanticTokensProvider.register(
				selector,
				new MainThreadDocumentSemanticTokensProvider(this._proxy, handle, legend, event)
			)
		);
	}
	...
	$registerDocumentRangeSemanticTokensProvider(
		...
		this._registrations.set(
			...
			this._languageFeaturesService.documentRangeSemanticTokensProvider.register(
				selector,
				new MainThreadDocumentRangeSemanticTokensProvider(this._proxy, handle, legend, event)
			)
		);
	}
	...
	$emitInlineCompletionProviderOptionsChange(
		handle: number,
		data: readonly IInlineCompletionProviderOptionDto[] | undefined
	): void {
		const obj = this._registrations.get(handle);
		if (obj instanceof ExtensionBackedInlineCompletionsProvider) {
						...
						edits: edits.map((edit): languages.DocumentPasteEdit => {
							return {
								...
								additionalEdit: edit.additionalEdit
									? reviveWorkspaceEditDto(edit.additionalEdit, this._uriIdentService, dataId =>
											this.resolveFileData(request.id, dataId)
										)
									: undefined
							};
						}),
			...
			return {
				edits: edits.map(edit => {
					return {
						...
						additionalEdit: reviveWorkspaceEditDto(edit.additionalEdit, this._uriIdentService, dataId =>
							this.resolveDocumentOnDropFileData(request.id, dataId)
						)
					};
				}),
				dispose: () => {
...
class ExtensionBackedInlineCompletionsProvider
	extends Disposable
	implements languages.InlineCompletionsProvider<IdentifiableInlineCompletions>
{
	...
	constructor(
		...
		this.setModelId = this._supportsSetModelId
			? async (modelId: string) => {
					await this._proxy.$handleInlineCompletionSetCurrentModelId(this.handle, modelId);
				}
			: undefined;

		this.setProviderOption = this._supportsSetProviderOption
			? async (optionId: string, valueId: string) => {
		...
		this.onDidChangeModelInfo = this._supportsOnDidChangeModelInfo
			? this._onDidChangeModelInfoEmitter.event
			: undefined;
		this.onDidProviderOptionsChange = this._supportsOnDidChangeProviderOptions
			? this._onDidProviderOptionsChangeEmitter.event
			: undefined;
	...
	public async handleItemDidShow(
	...
	public async handlePartialAccept(
		completions: IdentifiableInlineCompletions,
		item: IdentifiableInlineCompletion,
	...
	public async handleEndOfLifetime(
		completions: IdentifiableInlineCompletions,
		item: IdentifiableInlineCompletion,
		...
		function mapReason<T1, T2>(
			...
			f: (reason: T1) => T2
		): languages.InlineCompletionEndOfLifeReason<T2> {
		...
		if (this._supportsHandleEvents) {
			await this._proxy.$handleInlineCompletionEndOfLifetime(
				this.handle,
				completions.pid,
				item.idx,
	...
	public disposeInlineCompletions(
		completions: IdentifiableInlineCompletions,
		reason: languages.InlineCompletionsDisposeReason
	): void {
		this._proxy.$freeInlineCompletionsList(this.handle, completions.pid, reason);
	}

	public async handleRejection(
		completions: IdentifiableInlineCompletions,
		item: IdentifiableInlineCompletion
	): Promise<void> {
		if (this._supportsHandleEvents) {
