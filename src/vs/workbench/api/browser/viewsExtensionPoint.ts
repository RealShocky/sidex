...
const REMOTE = 'workbench.view.remote'; // Stub - remote feature removed
...
class ViewsExtensionHandler implements IWorkbenchContribution {
	...
	private addCustomViewContainers(
		...
		let auxiliaryBarOrder =
			100 +
			viewContainersRegistry.all.filter(
				v =>
					!!v.extensionId && viewContainersRegistry.getViewContainerLocation(v) === ViewContainerLocation.AuxiliaryBar
			).length +
			1;
					...
					if (viewContainer !== existingViewContainer) {
						viewsToMove.push(
							...this.viewsRegistry
								...
								.filter(view => (view as ICustomViewDescriptor).originalContainerId === descriptor.id)
						);
					}
				}
			...
			Object.entries(value).forEach(([key, value]) => {
				...
				for (let index = 0; index < value.length; index++) {
					...
					if (typeof item.icon === 'string') {
						icon =
							ThemeIcon.fromString(item.icon) || resources.joinPath(extension.description.extensionLocation, item.icon);
					}
					...
					if (
						isProposedApiEnabled(extension.description, 'contribAccessibilityHelpContent') &&
	...
	private removeViews(extensions: readonly IExtensionPointUser<ViewExtensionPointType>[]): void {
		...
		for (const viewContainer of this.viewContainersRegistry.all) {
			const removedViews = this.viewsRegistry
				...
				.filter(
					v =>
						(v as ICustomViewDescriptor).extensionId && removedExtensions.has((v as ICustomViewDescriptor).extensionId)
				);
	...
	private isValidViewDescriptors(
		...
		for (const descriptor of viewDescriptors) {
			...
			if (descriptor.contextualTitle && typeof descriptor.contextualTitle !== 'string') {
				collector.error(
					localize('optstring', 'property `{0}` can be omitted or must be of type `string`', 'contextualTitle')
				);
				return false;
			}
