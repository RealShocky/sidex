...
export class DiffEditorOptions {
	...
	public get editorOptions(): IObservableWithChange<IEditorOptions, { changedOptions: IEditorOptions }> {
	...
	constructor(
		...
		this.trueInlineDiffRenderingEnabled = derived(
			this,
...
function validateDiffEditorOptions(
	...
	return {
		...
		diffWordWrap: validateStringSetOption<'off' | 'on' | 'inherit'>(options.diffWordWrap, defaults.diffWordWrap, [
			'off',
			'on',
			'inherit'
		]),
